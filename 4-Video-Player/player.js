...
//! Render the playlist dynamically. Build playlist cards
for (let i = 1; i < playlistData.length; i++) {
  let mainDiv = createPlaylistCard(playlistData[i]);
...
//! The video card click from the playlist should load the video and data in the video player section.
playlistCards.forEach((card) => {
  card.addEventListener("click", function () {
    //? video-url
    let videoPlayer = document.getElementById("video-player");
    videoPlayer.src = `https://player.vimeo.com/video/${
      videoPlaySectionData[card.id - 1]["vimeoId"]
    }`;

    //? video views
    let viewsCount = document.getElementById("views-count");
    viewsCount.innerText = videoPlaySectionData[card.id - 1]["views"];
    ...
    videoDescription.innerText =
      videoPlaySectionData[card.id - 1]["description"];

    //! On the video card click, the current video should get the highlighted border.
    let allCards = document.getElementsByClassName("playlist-card");
...
//! Render the first video from the playlist automatically and dynamically.
document.addEventListener("DOMContentLoaded", () => {
  ...
  videoPlayer.attr(
    "src",
    `https://player.vimeo.com/video/${videoPlaySectionData[0]["vimeoId"]}`
  );

  //? video views
  let viewsCount = $("#views-count");
  viewsCount.text(videoPlaySectionData[0]["views"]);

  //? video title
  let videoTitle = $("#video-title");
  videoTitle.text(videoPlaySectionData[0]["title"]);

  //? video-description
  let videoDescription = $("#video-description");
  videoDescription.text(videoPlaySectionData[0]["description"]);

  //? add yellow border but before that remove yellow border from other card
  let allCards = $(".playlist-card");
